...
import { useAcceptInvite, useVerifyInvite } from '@/hooks/query/auth/useInvite'
const RULES = [
...
function SetPasswordContent() {
    ...
    const canSubmit = allPassed && matched && !accept.isPending
    ...
    if (!token) {
        return (
            <AuthShell
                ...
                subtitle="Open this page from the button in your invitation email — that link carries the code identifying your account."
                footer={
                    <a
                        href="/login"
    ...
    if (verifying) {
        return (
            <AuthShell title="Xenia" subtitle="Checking your invitation…">
    ...
    if (invalidToken) {
        return (
            <AuthShell
                ...
                subtitle="Invitation links expire after 72 hours and can only be used once. Ask the front desk or administrator to send you a new invitation."
                footer={
                    <a
                        href="/login"
                        className="inline-flex items-center gap-1.5 font-medium text-[#9A7140] transition-colors hover:text-[#7A5A32]"
    ...
    return (
        <AuthShell
            ...
            subtitle="Choose a password. You’ll use it every time you sign in."
            footer={
                <a
                    href="/login"
                    className="inline-flex items-center gap-1.5 font-medium text-[#9A7140] transition-colors hover:text-[#7A5A32]"
        ...
        >
            {invite?.email && (
                <div className="mb-5 flex justify-center">
                    <div className="inline-flex max-w-full items-center gap-2 rounded-full border border-[#E2D9C2] bg-white/60 px-3.5 py-1.5 text-[13px] font-medium text-[#3F3B32]">
                        <MailOpen01Icon
                            size={16}
                            className="shrink-0 text-[#4F6B52]"
                            strokeWidth={1.6}
                        />
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                >
                    {accept.isPending ? 'SAVING...' : 'SAVE PASSWORD'}
                    {!accept.isPending && <ArrowRight03Icon size={18} />}
                </button>

                <p className="text-center text-[13px] text-[#8A8271]">
                    This link works once and expires after 72 hours.
                </p>
            </form>
        </AuthShell>
    )
